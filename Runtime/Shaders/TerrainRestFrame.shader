// Terrain analogue of "VR/RestFrame". Structurally a URP TerrainLit shader (same passes
// as URP's TerrainLit): the ForwardLit pass renders the painted terrain as
// Surface A and blends it toward a second, complete PBR surface (B) using the global
// rest-frame anchored 4D noise field (RestFrameNoiseField.hlsl, the same field RestFrame
// uses). Only ForwardLit is customized; all other passes use stock URP terrain includes.
// Surface A (the terrain layers) is painted via the Terrain tool as usual; Surface B and
// the noise are edited in the material inspector (TerrainRestFrameShaderGUI).
// See TerrainRestFrameForwardPass.hlsl.
Shader "VR/TerrainRestFrame"
{
    Properties
    {
        // ---- Terrain (Surface A) - set by the terrain engine / painted via the Terrain tool ----
        ...
        // ---- Surface B - the surface the rest-frame noise reveals over the terrain ----
        [Toggle(_EMISSIONB)] _EmissionEnabledB("B - Use Emission", Float) = 0.0
        _BaseMapB("B - Base Map (RGB) Alpha (A)", 2D) = "white" {}
        _BaseColorB("B - Base Color", Color) = (1, 1, 1, 1)
        _BumpMapB("B - Normal Map", 2D) = "bump" {}
        _BumpScaleB("B - Normal Scale", Float) = 1.0
        _MetallicB("B - Metallic", Range(0.0, 1.0)) = 0.0
        _SmoothnessB("B - Smoothness", Range(0.0, 1.0)) = 0.5
        _OcclusionB("B - Occlusion", Range(0.0, 1.0)) = 1.0
        _EmissionMapB("B - Emission Map", 2D) = "white" {}
        [HDR] _EmissionColorB("B - Emission Color", Color) = (0, 0, 0)

        // ---- Noise Blend (same field as VR/RestFrame) ----
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Rest-frame noise field + Surface B (see TerrainRestFrameForwardPass.hlsl)
            // Noise primitive: Sine field / Classic Perlin 4D / Simplex 4D
            ...
            #pragma shader_feature_local_fragment _EMISSIONB

            #include "Packages/com.unity.render-pipelines.universal/Shaders/Terrain/TerrainLitInput.hlsl"
            #include "TerrainRestFrameForwardPass.hlsl"
            ENDHLSL
        }
    ...
    // Custom inspector for the Surface B + noise properties. Terrain layers (Surface A)
    // are still painted via the Terrain component as usual.
    CustomEditor "TerrainRestFrameShaderGUI"
